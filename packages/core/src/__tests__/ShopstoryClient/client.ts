...
test("throws error when default fetch finishes with error", async () => {
  const { mockedFn: consoleErrorMock, unmock } = mockConsoleMethod("error");
  ...
  await expect(shopstoryClient.build()).rejects.toThrowError(
    'Above error occurred in default fetch function for type "product". This is probably Shopstory error.'
  );
...
test("throws error when default fetch doesn't resolve remaining unresolved resources returned from custom fetch", async () => {
...
test("throws error when post fetch finishes with error", async () => {
  const { mockedFn: consoleErrorMock, unmock } = mockConsoleMethod("error");
...
test("throws error when transformer finishes with error", async () => {
  const { mockedFn: consoleErrorMock, unmock } = mockConsoleMethod("error");
