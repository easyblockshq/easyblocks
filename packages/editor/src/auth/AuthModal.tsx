import { ShopstoryIcon, SSColors, Stack } from "@easyblocks/design-system";
import {
  SupabaseClient,
  useSession,
  useUser,
...
function AuthModal() {
  ...
  const [view, setView] = useState<
    "sign_in" | "sign_up" | "password_recovery" | "password_update"
  >(() => state?.view ?? "sign_up");

  const session = useSession();
  const navigate = useNavigate();

  if (session && view !== "password_update") {
    return <Navigate to="/" />;
  }

  const redirectUrl = removeHashFromUrl(window.location.href);
...
const FormSecondaryButton = styled.button`
  all: unset;
  width: 100%;
  height: 44px;
  background: #fff;
  ...
  return (
    ...
    >
      {formStatus.status !== "success" && (
        <Fragment>
          <FormTitle>Create your Shopstory account</FormTitle>
          ...
          <Stack gap={24}>
            <FormSubmitButton disabled={formStatus.status === "loading"}>
              Create account
            </FormSubmitButton>
...
function SignInForm(props: {
  supabaseClient: SupabaseClient;
  onViewChange: (view: "sign_up" | "password_recovery") => void;
}) {
  const [formStatus, setFormStatus] = useState<
    | { status: "idle"; error: null }
    | { status: "loading"; error: null }
  ...
  return (
    ...
    >
      <FormTitle>Log in to Shopstory</FormTitle>
      ...
      <FormSecondaryActions>
        <FormLink
          ...
          onClick={(event: MouseEvent) => {
            event.preventDefault();
            props.onViewChange("password_recovery");
          }}
        >
          Reset password
        </FormLink>
    ...
    >
      {formStatus.status !== "success" && (
        <Fragment>
          ...
          <FormSubmitButton type="submit" style={{ width: "100%" }}>
            Reset password
          </FormSubmitButton>
        ...
        <Fragment>
          <Stack gap={24}>
            ...
            <div
              ...
              `}
            >
              If an account exists for {formStatus.email}, you will get an email
      ...
      onClick={() => {
        props.supabaseClient.auth
          .signInWithOAuth({
            provider: "google",
            options: {
              redirectTo: removeHashFromUrl(window.location.href),
            },
          })
          .then((authResponse) => {
            if (authResponse.error) {
