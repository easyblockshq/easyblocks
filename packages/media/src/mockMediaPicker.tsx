...
const PLAYGROUND_MESSAGE = `This is the playground media picker. In production Shopstory would connect to the media picker of your CMS.`;
...
const Component = (
  ...
  const shouldUseApi = apiClient !== undefined && projectId !== undefined;
  ...
  const inputFileElement = shouldUseApi ? (
    <input
      type="file"
      onChange={(event) => {
  ...
  return (
    <div>
      {!shouldUseApi && <FieldMessage>{PLAYGROUND_MESSAGE}</FieldMessage>}
      ...
      >
        ...
        >
          ...
          >
            {!shouldUseApi && (
              <Notice>
                <strong>Important!</strong> {PLAYGROUND_MESSAGE}
              </Notice>
            )}

            {shouldUseApi && items !== null && items.length > 0 && (
              <SSButtonPrimary
                component="label"
                icon={SSIcons.Add}
        ...
        apiClient && projectId
          ? () => {
              return apiClient.assets
                .getAssets({ projectId, type: key })
                .then((assets) => {
                  return assets.map(mediaAdapter);
                });
            }
          : fetcher;
